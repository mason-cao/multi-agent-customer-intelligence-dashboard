...
If the workspace record or database file does not exist, the backend returns a 404. The frontend API client clears the active workspace from `localStorage` and returns the user to `/workspaces`.
...
**Source Tables** (generated by `backend/app/services/data_generation/`, with a CLI in `scripts/generate_data.py`):
...
Stage 14: AuditAgent + QueryAgent
          ├── AuditAgent reads: all tables → writes: audit_results
          └── QueryAgent: on-demand NL query processing
```

### Reliability & Degraded Completion

The pipeline is a declarative spec in `services/pipeline.py` (`PIPELINE`), where each agent is an `AgentSpec` carrying a `critical` flag. Both the workspace generator and CLI use `execute_pipeline()`, which inspects each agent's `_status`:
...
All agents in an invocation share one `run_id`. Each stage replaces its output and writes its audit record in the same transaction; execution failures roll back output changes before recording failure. Partial validation results remain available with warnings. Source generation also commits all seven tables atomically and releases each generated frame before creating the next.

Missing or unreadable audit inputs produce failed findings. The Agent Audit endpoint selects the latest run per agent in SQL, including failures, instead of loading all successful history and hiding recent failures.
...
Each executed agent's outcome (status + duration + tokens) is logged to `agent_runs` and surfaced on the **Agent Audit** page, making the lineage of each insight legible.
...
POST /api/workspaces/{id}/generate
  ...
  │
  │   Stages 1-7: Data Generation (services/data_generation/)
...
- **Server state**: TanStack Query manages fetching and cancellation. `useWorkspaceQuery` scopes dashboard keys to workspace ID and completion time; cache clearing cancels matching requests. Request credentials are captured so retries and delayed errors cannot affect another active workspace.
