...
from app.services.scenarios import SCENARIOS, generate_random_scenario
...
def init_metadata_db():
    ...
    WorkspaceBase.metadata.create_all(bind=workspace_db.metadata_engine)
    # Lightweight column migrations for existing DBs
    from sqlalchemy import inspect, text
    columns = [c['name'] for c in inspect(workspace_db.metadata_engine).get_columns('workspaces')]
    migrations = {
    ...
    for column, ddl in migrations.items():
        if column not in columns:
            with workspace_db.metadata_engine.begin() as conn:
